...
class ThermostatDevice extends Homey.Device {
    ...
    onInit() {
      ...
      // do not call addCapability on every init!
        ...
        var measure_old = Number(this.getCapabilityValue('measure_temperature'))
    ...
    _sync() {
      ...
      zone_data.forEach(function(value){
        ...
        if ( value.zoneId == id) {
            ...
            if (typeof device.getCapabilityValue('measure_temperature') !== 'undefined') {
              var measure_old = device.getCapabilityValue('measure_temperature')
            }
            // TEST
            //console.log('battery_alrm test  ')
            //device.sbilityValue('alarm_battery', true)
            //  .catch(device.error);
            // TEST END
            //console.log('active faults: ' + value.name + ' ' + value.activeFaults.length);
            //console.log(value.activeFaults.length)
            if ( value.activeFaults.length == 0 ) {
            ...
            if (typeof value.activeFaults[0] !== 'undefined') {
              console.log('active faults: ' + value.name + ' ' + JSON.stringify(value.activeFaults));
              console.log(value.activeFaults[0].faultType)
              Object.keys(value.activeFaults).forEach(function(key) {
                  console.log(value.activeFaults[key])
                  if (value.activeFaults[key].faultType == 'TempZoneActuatorLowBattery') {
            ...
            if ( typeof value.temperatureStatus.temperature !== 'undefined' && typeof measure_old !== 'undefined' && measure_old != value.temperatureStatus.temperature) {
        ...
        //Homey.log('Response: ' + result.data)
            ...
            if (installation == "None") {
              ...
              http.get(options).then(function (result) {
                  ...
                  console.log ('locationid opgeslagen insettings');
                  resolve('ok')
              })
              .catch(function(reject) {
                console.log(reject);
