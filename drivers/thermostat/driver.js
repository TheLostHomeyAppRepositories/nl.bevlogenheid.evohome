  ...
  onPairListDevices( data, callback ) {
    ...
    return new Promise (function ( resolve, reject ) {
      ...
      evohomelogin(evohomeUser,evohomePassword,appid).then(function() {
        ...
        console.log('--- locationId as stored in settings --- ');
        console.log(Homey.ManagerSettings.get('locationId'));
        console.log('--- Settings locationId ---')
        var locationId= Homey.ManagerSettings.get('locationId');
        console.log('driver.js onpair locationid')
        console.log(locationId)

        var promises = locationId.map(async (value) => {
          //console.log(zones_read.length);
          var result = await list_devices_locations(value)
          //console.log('pair devices result')
          //console.log(result)
          //console.log('global list')
          //console.log(global_list_devices);
          return new Promise((res, rej) => {res(result)})
        })

        Promise.all(promises)
        .then((results) => {
          console.log('done with pairing')
          devices = global_list_devices;
          console.log('promise list device all')
          console.log(global_list_devices);
          console.log('end promise list device all')
          resolve(global_list_devices);
    ...
    http.get(options).then(function (result) {
        ...
        //Homey.log('Response: ' + result.data)
            ...
            if (installation == "None") {
              ...
              http.get(options).then(function (result) {
                  var data = JSON.parse(result.data)
                  console.log('--- driver.js system id check ---- ' )
                  //console.log(data)
...
var list_devices_locations = (value) => {
  return new Promise((resolve,reject) => {
    console.log('list_devices_locations: ' + value);
    var access_token = Homey.ManagerSettings.get('access_token');
    ...
    http.get(options).then(function (result) {
      ...
      zones.forEach((device) => {
        ...
        global_list_devices.push(foundDevice);
        console.log(global_list_devices.length)
      resolve(global_list_devices)
    })
  })
})
